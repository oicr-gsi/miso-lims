...
public class SQLSampleDAO implements SampleStore {
   ...
   public static final String SAMPLES_SELECT_BY_SEARCH = SAMPLES_SELECT + " WHERE " + "identificationBarcode LIKE ? OR "
         + "name LIKE ? OR " + "alias LIKE ? OR " + "description LIKE ? OR " + "scientificName LIKE ? ";
      ...
      if (sample.getId() == AbstractSample.UNSAVED_ID) {
         ...
         if (!sampleNamingScheme.allowDuplicateEntityNameFor("alias") && !listByAlias(sample.getAlias()).isEmpty()) {
            ...
            try {
               ...
               if (sampleNamingScheme.validateField("name", sample.getName())
                     && sampleNamingScheme.validateField("alias", sample.getAlias())) {
