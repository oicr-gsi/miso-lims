...
public class SQLExperimentDAO implements ExperimentStore {
   ...
   public long save(Experiment experiment) throws IOException {
      ...
      if (experiment.getId() == AbstractExperiment.UNSAVED_ID) {
         ...
         /*
          * params.addValue("experimentId", experiment.getExperimentId()) .addValue("name", experiment.getName());
          * NamedParameterJdbcTemplate namedTemplate = new NamedParameterJdbcTemplate(template); namedTemplate.update(EXPERIMENT_UPDATE,
          * params);
          */
      }
