...
public class SQLRunDAO implements RunStore {
   ...
   public synchronized int[] saveAll(Collection<Run> runs) throws IOException {
      ...
      for (Run run : runs) {
         ...
         try {
            MapSqlParameterSource params = new MapSqlParameterSource();
            params.addValue("accession", run.getAccession()).addValue("alias", run.getAlias())
                  .addValue("description", run.getDescription()).addValue("platformRunId", run.getPlatformRunId())
