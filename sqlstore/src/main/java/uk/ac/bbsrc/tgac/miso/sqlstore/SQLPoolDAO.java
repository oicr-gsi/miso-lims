...
public class SQLPoolDAO implements PoolStore {
   ...
   public static final String POOL_SELECT_BY_RELATED_LIBRARY = "SELECT DISTINCT pool.* " + "FROM Library li "
         ...
         "INNER JOIN " + TABLE_NAME + " pool ON pool.poolId = pld.pool_poolId " + "OR pool.poolId = ple.pool_poolId "
         + "WHERE li.libraryId=?";
         ...
         try {
            ...
            if (namingScheme.validateField("name", pool.getName())) {
               ...
               if (newId.longValue() != pool.getId()) {
                  ...
                  new NamedParameterJdbcTemplate(template).update(POOL_DELETE,
                        new MapSqlParameterSource().addValue("poolId", newId.longValue()));
