...
public class SQLPlateDAO implements PlateStore {
   ...
   public long save(Plate<? extends List<? extends Plateable>, ? extends Plateable> plate) throws IOException {
      ...
      if (plate.getId() == AbstractPlate.UNSAVED_ID) {
          ...
          * params.addValue("identificationBarcode", name + "::" + plate.getTagBarcode()); Number newId =
          * insert.executeAndReturnKey(params); plate.setPlateId(newId.longValue()); plate.setName(name);
          */
      } else {
         if (plate.getTagBarcode() != null) {
            params.addValue("tagBarcodeId", plate.getTagBarcode().getId());
         ...
         try {
            ...
            if (namingScheme.validateField("name", plate.getName())) {
               params.addValue("plateId", plate.getId()).addValue("name", plate.getName()).addValue("description", plate.getDescription())
                     .addValue("identificationBarcode", plateBarcode).addValue("locationBarcode", plate.getLocationBarcode());
               NamedParameterJdbcTemplate namedTemplate = new NamedParameterJdbcTemplate(template);
         ...
         /*
          * params.addValue("plateId", plate.getPlateId()); params.addValue("name", plate.getName());
          * params.addValue("identificationBarcode", plate.getName() + "::" + plate.getTagBarcode()); NamedParameterJdbcTemplate
          * namedTemplate = new NamedParameterJdbcTemplate(template); namedTemplate.update(PLATE_UPDATE, params);
          */
      }
