...
public class SQLSequencerPartitionContainerDAO implements SequencerPartitionContainerStore {
   ...
   public synchronized long save(SequencerPartitionContainer<SequencerPoolPartition> sequencerPartitionContainer) throws IOException {
      ...
      if (sequencerPartitionContainer.getId() == AbstractSequencerPartitionContainer.UNSAVED_ID) {
          ...
          * else { throw new IOException("Cannot save SequencerPartitionContainer - invalid field:" +
          * sequencerPartitionContainer.toString()); }
          * 
          * } catch (MisoNamingException e) { throw new IOException("Cannot save SequencerPartitionContainer - issue with naming scheme",
          * e); }
          */
          ...
          * else { throw new IOException("Cannot save SequencerPartitionContainer - invalid field:" +
          * sequencerPartitionContainer.toString()); } } catch (MisoNamingException e) { throw new
          * IOException("Cannot save SequencerPartitionContainer - issue with naming scheme", e); }
          */
    ...
    * ArrayList<SequencerPoolPartition>(partitionDAO.listBySequencerPartitionContainerId(id)); for (SequencerPoolPartition part :
    * partitions) { part.setSequencerPartitionContainer(s); } s.setPartitions(partitions);
    * 
    ...
    * s.setSecurityProfile(securityProfileDAO.get(rs.getLong("securityProfile_profileId"))); } catch (IOException e1) {
    * e1.printStackTrace(); }
    * 
