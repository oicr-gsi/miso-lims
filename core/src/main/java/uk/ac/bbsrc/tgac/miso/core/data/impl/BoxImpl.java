...
public class BoxImpl extends AbstractBox {
  ...
  public boolean isValidPosition(String position) {
    ...
    if (BoxUtils.getNumberForChar(position.charAt(0)) > getSize().getRows()) return false;
    int col = BoxUtils.tryParseInt(position.substring(1, 3));
    if (col <= 0 || col > getSize().getColumns()) return false;
    return true;
  }
  ...
  private void validate(String position) {
    ...
    if (BoxUtils.getNumberForChar(position.charAt(0)) > getSize().getRows()) throw new IndexOutOfBoundsException("Row letter too large!");
    int col = BoxUtils.tryParseInt(position.substring(1, 3));
