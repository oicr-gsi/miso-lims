...
public class TgacDataObjectFactory extends DataObjectFactory {
  ...
  @Override
  public Box getBoxOfSize(int rows, int columns) {
    return new BoxImpl(rows, columns);
  }

  @Override
  public Box getBoxOfSize(int rows, int columns, User user) {
    return new BoxImpl(rows, columns, user);
  }
}
