...
public class TgacDataObjectFactory extends DataObjectFactory {
   ...
   public Pool<? extends Poolable> getPoolOfType(PlatformType platformtype, User user) throws IllegalArgumentException {
      if (platformtype != null) {
         ...
         return p;
         /*
          * if (platformtype.equals(PlatformType.ILLUMINA)) { return getIlluminaPool(user); } else if
          * (platformtype.equals(PlatformType.LS454)) { return getLS454Pool(user); } else if (platformtype.equals(PlatformType.SOLID)) {
          * return getSolidPool(user); } else if (platformtype.equals(PlatformType.PACBIO)) { return getPacBioPool(user); } else { throw new
